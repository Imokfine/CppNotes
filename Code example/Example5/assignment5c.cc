    ...
    if (world.rank() == 0) {
        ...
        write_to_file(std::string("serial_mpi.txt"), CH.get_hull()); 		// Write hull points to file
          ...
          */
            ...
            std::vector<Point> points_rank0(std::begin(points), std::begin(points) + len);
            std::vector<Point> points_rank1(std::begin(points) + len, std::begin(points) + len * 2);
            std::vector<Point> points_rank2(std::begin(points) + len * 2, std::end(points));
