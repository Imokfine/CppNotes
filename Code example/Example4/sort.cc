...
int main()
{
	...
	std::for_each(A.begin(), A.end(), [](Coord const& c) {std::cout << c.c << std::setw(9) << c.x << std::setw(9) << c.y << "\n"; });

	// Sort by-x-then-y
	std::sort(A.begin(), A.end(), [](Coord const& a, Coord const& b) {return a.x < b.x || (a.x == b.x && a.y < b.y);});
	std::cout << "\nAfter sorting x_then_y\n";
	std::for_each(A.begin(), A.end(), [](Coord const& c) {std::cout << c.c << std::setw(9) << c.x << std::setw(9) << c.y << "\n";});

	// Sort by-y-then-x
	std::sort(A.begin(), A.end(), [](Coord const& a, Coord const& b) {return a.y < b.y || (a.y == b.y && a.x < b.x); });
